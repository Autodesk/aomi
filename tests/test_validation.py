...
class StringTests(unittest.TestCase):
    ...
    def test_is_string(self):
        assert aomi.validation.is_plain_string("foo") == None
        assert aomi.validation.is_plain_string("70758F21-946C-4C14-AD67-53DDCA5C9F4B") == None
        with self.assertRaises(aomi.exceptions.Validation):
            aomi.validation.is_plain_string(self.ghost_emoji)
        with self.assertRaises(aomi.exceptions.Validation):
            aomi.validation.is_plain_string(self.some_binary)
